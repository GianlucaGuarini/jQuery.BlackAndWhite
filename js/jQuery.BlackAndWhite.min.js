...
pic.src,$img=$imageWrapper.find("img"),css={"position":"absolute",top:0,left:0,display:invertHoverEffect?"none":"block"};if(supportsCanvas&&!cssfilters){var width=$img.width(),height=$img.height(),realWidth=pic.width,realHeight=pic.height;$('<canvas class="BWfade" width="'+realWidth+'" height="'+realHeight+'"></canvas>').prependTo($imageWrapper);var $canvas=$imageWrapper.find("canvas");$canvas.css($.extend(css,{width:width,height:height}));_manipulateImage(pic,$canvas[0],realWidth,realHeight)}else{var ieWidth=
$img.prop("width");var ieHeight=$img.prop("height");css[cssPrefix("Filter")]="grayscale(100%)";$("<img src="+src+' width="'+ieWidth+'" height="'+ieHeight+'" class="BWFilter BWfade" /> ').prependTo($imageWrapper);$(".BWFilter").css($.extend(css,{"filter":"progid:DXImageTransform.Microsoft.BasicImage(grayscale=1)"}))}};this.init=function(options){$container.each(function(index,tmpImageWrapper){var pic=new Image,$imageWrapper=$(tmpImageWrapper);pic.src=$imageWrapper.find("img").prop("src");if(!pic.width)$(pic).on("load",
function(){_injectTags(pic,$imageWrapper)});else _injectTags(pic,$imageWrapper)});if(BnWWorker)_webWorkerLoop();if(hoverEffect){$container.on("mouseleave",_onMouseLeave);$container.on("mouseenter",_onMouseEnter)}if(responsive)$window.on("resize orientationchange",$container.resizeImages)};this.resizeImages=function(){$container.each(function(index,currImageWrapper){var pic=$(currImageWrapper).find("img:not(.BWFilter)");var currWidth,currHeight;if(isIE7){currWidth=$(pic).prop("width");currHeight=$(pic).prop("height")}else{currWidth=
$(pic).width();currHeight=$(pic).height()}$(this).find(".BWFilter, canvas").css({width:currWidth,height:currHeight})})};return this.init(options)}})})(jQuery);